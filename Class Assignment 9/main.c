/* 9 : Write a C program implementing Quick Sort. Run the program with  same  set of random numbers as in Quick sort. Count the number of comparisons in each case.
 */
...
int partition(int arr[], int lpw, int high, long long *comparisons);
...
int main()
{
    ...
    for (int run = 1; run <= num_runs; run++)
    {
        ...
        // Sort the array using Quick Sort and count comparisons
        long long comparisons = quickSort(random_numbers, 0, setSize);
...
long long quickSort(int arr[], int low, int high)
{
    ...
    if (low < high)
    {
        ...
        // Recursively sort the sub-arrays and count comparisons
